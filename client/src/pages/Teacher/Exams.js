  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{backgroundImage: 'url(/BG.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat'}}>
      ...
      <div className="max-w-5xl mx-auto p-8 relative z-10">
        <h1 className="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2"><ClipboardList className="text-purple-600" /> Exams</h1>
      {loading ? <div className="text-gray-300">Loading...</div> : exams.length === 0 ? <div className="text-gray-300">No exams found.</div> : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {exams.map(exam => (
            <div key={exam._id} className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/50 flex flex-col gap-2">
