...
const ClassSectionStudents = () => {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{backgroundImage: 'url(/BG.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat'}}>
      ...
      <button
        className="mb-6 flex items-center gap-2 px-4 py-2 border border-purple-500/30 rounded-lg text-purple-300 font-semibold bg-gray-800/50 hover:bg-gray-700/50 transition shadow-sm"
        onClick={() => navigate(-1)}
      >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        <div className="flex-1 flex items-center gap-2">
          ...
          <input type="date" value={date} onChange={e => setDate(e.target.value)} className="input-dark border-purple-500/30 rounded px-3 py-2" />
        </div>
      ...
      {loading ? (
        ...
        <div className="p-8 text-center text-gray-300">No students in this section.</div>
      ) : (
        <div className="space-y-4 mb-12"> {/* Add extra margin below student list */}
          {students.map(stu => (
            <div key={stu._id} className="flex items-center justify-between card-purple rounded-xl shadow-lg p-4 border border-purple-500/30 transition-all duration-200">
              <div>
                <div className="font-semibold text-lg text-white">{stu.firstName} {stu.lastName}</div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  className={`px-4 py-2 rounded-l-lg font-semibold border transition-all duration-150 ${attendance[stu._id] === 'present' ? 'bg-green-600 text-white border-green-600' : 'bg-gray-700/50 text-gray-200 border-gray-500/30 hover:bg-green-900/50'}`}
                ...
                <button
                  className={`px-4 py-2 rounded-r-lg font-semibold border transition-all duration-150 ${attendance[stu._id] === 'absent' ? 'bg-red-600 text-white border-red-600' : 'bg-gray-700/50 text-gray-200 border-gray-500/30 hover:bg-red-900/50'}`}
      ...
      <div className="fixed bottom-0 left-0 w-full bg-gradient-to-t from-gray-900/90 to-gray-900/0 p-4 flex justify-center z-20 sm:static sm:bg-none sm:p-0">
        <button
          className="btn-dark-primary w-full max-w-xs py-3 text-lg font-bold rounded-xl shadow-lg transition-all duration-150 disabled:opacity-60"
